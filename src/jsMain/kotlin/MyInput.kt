...
class MyInput(
    ...
    private fun handle_mousemove(evt: MouseEvent) {
        ...
        if (lastclick_button == 3) {
            soundDataSource.set_volume((mouse_pos.x - vol_bar.offset_x) / vol_bar.width)
        }
    ...
    fun handle_mousedown(evt: MouseEvent) {
        ...
        if (mouse_pos.x >= vol_bar.offset_x && mouse_pos.y >= vol_bar.offset_y &&
            mouse_pos.x <= vol_bar.offset_x + vol_bar.width && mouse_pos.y <= vol_bar.offset_y + vol_bar.height
        ) {
            soundDataSource.set_volume((mouse_pos.x - vol_bar.offset_x) / vol_bar.width)
            lastclick_button = 3
        }
