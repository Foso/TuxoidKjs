...
// Private:
	var that = this;
	var ktgame = new untitled23.KtGame(that);
	...
	this.savegame = new untitled23.SaveGame();
	...
	this.clear_savegame = function(){
    		ktgame.clear_savegame()
    	}

    this.update_savegame = function(lev, steps){
        ktgame.update_savegame(lev, steps)
    }

    this.store_savegame = function(){
        return ktgame.store_savegame();
    }

    this.retrieve_savegame = function(uname, pass){
       return ktgame.retrieve_savegame(uname,pass,that);

    }

    this.name_savegame = function(uname, pass){
    return ktgame.name_savegame(uname,pass,that);

    }

    this.change_password = function(pass, newpass){
        return ktgame.change_password(pass, newpass);
    }
    ...
    this.dbxcall_save = function(uname, pass){
        return ktgame.dbxcall_save(uname,pass);
    }

    this.dbxcall_load = function(uname, pass){
        return ktgame.dbxcall_load(uname,pass);
    }

    this.dbxcall_chpass = function(pass, newpass){
        return ktgame.dbxcall_load(pass, newpass);
    }

	this.load_level = function(lev_number){
	untitled23.load_level(lev_number,that);
	...
	this.walkable = function(curr_x, curr_y, dir){
		return ktgame.walkable(curr_x,curr_y,dir,that,this)
	}
	
	this.start_move = function(src_x, src_y, dir){
	    untitled23.start_move(src_x,src_y,dir,that);
	}
	...
	this.dir_to_coords = function(curr_x, curr_y, dir){
		return untitled23.dir_to_coords(curr_x,curr_y,dir)
	}
	
	this.opposite_dir = function(dir){
		return untitled23.opposite_dir(dir);
	}
	
	this.get_adjacent_tiles = function(tile_x, tile_y){// Potential for optimization
		return untitled23.get_adjacent_tiles(tile_x,tile_y,that);
	}
	
	this.is_in_bounds = function(tile_x, tile_y){
		return ktgame.is_in_bounds(tile_x,tile_y);
	}
	
	this.can_see_tile = function(eye_x, eye_y, tile_x, tile_y){
	return untitled23.can_see_tile(eye_x, eye_y, tile_x, tile_y);
	...
	this.next_level = function(){
		untitled23.next_level(that);
	}
	
	this.reset_level = function(){
		untitled23.reset_level(that);
	}
	
	this.play_sound = function(id){
		untitled23.play_sound(id,that);
		// Useful commands
		//audioElement.pause();
	...
	this.set_volume = function(vol){
		//TODOuntitled23.set_volume();
	}
	
	this.toggle_sound = function(){
		untitled23.toggle_sound(that)
	}
	...
	this.remove_soundrestriction = function(){
		untitled23.remove_soundrestriction(that)
	}
	
	this.toggle_single_steps = function(){
	untitled23.toggle_single_steps();

	}
	
	this.toggle_paused = function(){
	untitled23.toggle_paused(that);
